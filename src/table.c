...
static Entry* findEntry(Entry* entries, int capacity, ObjString* key) {
    ...
    for (;;) {
        ...
        // TODO: Linearly probe for now; implement a more robust algorithm
        if (entry->key == NULL) {
...
static void adjustCapacity(Table* table, int capacity) {
    ...
    // Re-populate the hash table after reallocation to avoid re-collisions
    table->count = 0;
...
// Add given key-value pair to hash table and returns if succeed
...
bool tableDelete(Table* table, ObjString* key) {
    ...
    entry->key = NULL;
    entry->value = BOOL_VAL(true); // Place a tombstone to allow probing
    return true;
}
